...
function exportRecordingSegment(frames, sampleRate, segIndex, segCount, stamp) {
    ...
    if (segCount > 1) {
        warnings.push("The time/div changed mid-recording; this is segment " + segIndex + " of " +
            segCount + ", each written at its own samplerate.");
    }
...
function exportRecordingSR() {
    ...
    if (segments.length > 1) {
        log("The time/div changed during the recording; writing " + segments.length +
            " files, one per samplerate (a .sr carries only one).");
        showMessage("Time/div changed - saving " + segments.length + " files", "ALL");
    }
